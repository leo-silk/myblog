...
function hot_comment_viewed($number, $days){
    ...
    foreach ($posts as $post){
		$post_title = mb_strimwidth($post->post_title, 0, 30, '...');
        $output .= "\n<li><span class='li-icon li-icon-$i'>$i.</span><a href= \"".get_permalink($post->ID)."\" rel=\"bookmark\" title=\"".$post->post_title." (".$post->comment_count."条评论)\" >$post_title</a><span class=\"list-date\">".$post->comment_count."</span></li>";
		$i++;
    }
    echo $output;
}

...
function get_timespan_most_viewed($mode = '', $limit = 10, $days = 7, $display = true) {
	...
	if($most_viewed) {
		...
		foreach ($most_viewed as $post) {
			$post_title = mb_strimwidth(get_the_title(), 0, 28, '...');
			$post_views = intval($post->views);
			$post_views = number_format($post_views);
			$temp .= "<li><span class='li-icon li-icon-$i'>$i.</span><a href=\"".get_permalink()."\" title=\"".get_the_title()."\">$post_title</a><span class=\"list-date\">$post_views</span>".__('', 'wp-postviews')."</li>";
			$i++;
		}
	} else {
